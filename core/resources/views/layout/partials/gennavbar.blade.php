                    ...
                    <ul>
                        ...
                        <li>
                            <div class="support-search-area">
                                <form
                                    action="{{url('/').'/shop'.'/'.Request::route('category').'/'.Request::route('subcategory')}}"
                                    class="search-form">
                                    <div class="form-element has-icon" action="{{route('user.search')}}">
                                        <input name="term" type="text" class="input-field"
                                               placeholder="Search your keyword" value="{{request()->input('term')}}">
                                        <div class="the-icon">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link" href="#" style="text-align: center" id="navbarDropdown" role="button"
                               data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <img src=" {{asset('assets/custom/customer_care.png')}}" width="25"/>
                                <span class="badge d-block">Need Help?</span>

                            </a>
                            <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                <a class="dropdown-item" href="#"> {{$gs->support_phone}}</a>
                                <a class="dropdown-item" href="#"> {{$gs->support_email}}</a>
                        ...
                        @auth ('vendor')
                            <li class="nav-item dropdown">
                                <a class="nav-link" href="#" style="text-align: center" id="shopDropdown" role="button"
                                   data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    <img src=" {{asset('assets/custom/shop.png')}}" width="25"/>
                                ...
                                <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                    ...
                                    {{--                                    <a href="{{route('vendor.product.manage')}}" class="dropdown-item">Manage--}}
                                    {{--                                        Products</a>--}}
                                    {{--                                    <a href="{{route('vendor.product.create')}}" class="dropdown-item">Upload--}}
                                    {{--                                        Product</a>--}}
                                    {{--                                </div>--}}
                                    {{--                            </li>--}}
                                    {{--                            <li class="nav-item dropdown">--}}
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link " style="text-align: center" href="#" id="accountDropdown"
                               role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <img src="{{asset('assets/custom/user.png')}}" width="25"/>
                                ...
                                @if (!Auth::check() && !Auth::guard('vendor')->check())
                                  ...
                                  @elseif (Auth::guard('vendor')->check())
                                    {{-- <a class="nav-link dropdown-toggle" href="#"
                                       data-toggle="dropdown">{{Auth::guard('vendor')->user()->shop_name}}</a>
                                     <div class="dropdown-menu"> --}}
                                        <a href="{{route('vendor.setting')}}" class="dropdown-item">Settings</a>
                                        <a href="{{route('vendor.changePassword')}}" class="dropdown-item">Change
                                            Password</a>
                                        ...
                                        <a href="{{route('vendor.logout', Auth::guard('vendor')->user()->id)}}"
                            ...
                            @if (!Auth::guard('vendor')->check() || request()->is('product/*/details') || request()->is('cart'))
                                <li class="cart" id="cart" style="text-align: center">
                                    ...
                                    <a href="#">
                                        ...
                                        <img src=" {{asset('assets/custom/cart.png')}}" width="25"/>
            ...
            <div class="cart-products">
                ...
                <div class="">
                    ...
                    @foreach ($carts as $cart)
                        <div class="single-product-item" id="singleitem{{$cart->id}}">
                            <!-- single product item -->
                            <div class="thumb">
                                <img
                                    src="{{asset('assets/user/img/products/') . '/' . \App\Product::find($cart->product_id)->previewimages()->first()->image}}"
                                    alt="recent review">
                            </div>
                            <div class="content">
                                <a href="{{route('user.product.details', [$cart->product->slug, $cart->product->id])}}">
                                    <h4 class="title">{{strlen($cart->title) > 18 ? substr($cart->title, 0, 18) . '...' : $cart->title}}</h4>
                                </a>
                                <div class="price" style="font-size:12px;">
                                    @if (empty($cart->current_price))
                                        <span class="pprice"
                                              id="price{{$cart->id}}">{{$gs->base_curr_symbol}} {{$cart->price}}</span>
                                        <span class="sidequantity"
                                              id="quantity{{$cart->id}}">({{$cart->quantity}})</span>
                                    @else
                                        <span class="pprice"
                                              id="price{{$cart->id}}">{{$gs->base_curr_symbol}} {{$cart->current_price}}</span>
                                        <del id="delprice{{$cart->id}}"
                                             class="dprice">{{$gs->base_curr_symbol}} {{$cart->price}}</del> <span
                                            class="sidequantity" id="quantity{{$cart->id}}">({{$cart->quantity}})</span>
                                    @endif
                                </div>
                                @php
                                ...
                                <a style="font-size:12px;" href="#" class="remove-cart"
                                   @click="precartlen--;removeproduct({{$cart->id}})">Remove</a>
                            </div>
                    ...
                    <div class="single-product-item" v-for="(product, index) in products">
                        ...
                        <div class="content">
                            ...
                            <div style="font-size:12px;" class="price" v-if="!product.current_price"><span
                                    class="pprice" :id="'price'+product.cart_id">{{$gs->base_curr_symbol}} @{{product.price * product.quantity}}</span>
                            ...
                            <div style="font-size:12px;" class="price" v-if="product.current_price"><span class="pprice"
                                                                                                          :id="'price'+product.cart_id">{{$gs->base_curr_symbol}} @{{product.current_price * product.quantity}}</span>
                                <del class="dprice" :id="'delprice'+product.cart_id">{{$gs->base_curr_symbol}}
